...
class VectorGridGeoJson(JSCSSMixin, Layer):
    """
    Add vector tile layers based on https://github.com/zarandras/Leaflet.VectorGrid.

    Parameters
    ----------
    url: url to tile provider
        e.g. https://free-{s}.tilehosting.com/data/v3/{z}/{x}/{y}.geojson?token={token}
    layer_name: string, default "VectorGridGeoJsonLayer"
        name of the layer
    options: dict or str, VectorGrid.geojson options

        For convenience you can pass VectorGrid.geojson options as python dictionary or string.
    ...
    >>> m = folium.Map()
    >>> url = "https://free-{s}.tilehosting.com/data/v3/{z}/{x}/{y}.geojson?token={token}"
    >>> options = {
    ...
    >>> VectorGridGeoJson(url,"layer_name",options).add_to(m)
    ...
    _template = Template(
        """
            ...
            var {{ this.get_name() }} = L.vectorGrid.geojson(
                '{{ this.url }}',
    ...
    def __init__(self, url, layer_name, options=None, show=True, overlay=True):
        self.layer_name = layer_name if layer_name else "VectorGridGeoJsonLayer"

        super(VectorGridGeoJson, self).__init__(
            name=self.layer_name, show=show, overlay=overlay
        )

        self.url = url
        self._name = "VectorGridGeoJson"
